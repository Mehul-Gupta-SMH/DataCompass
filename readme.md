...
Copy the template and fill in your keys — this file is gitignored and never committed:

```bash
cp APIManager/model_access_config.YAML.example APIManager/model_access_config.YAML
```
...
Before generating queries, you need to load your database schema into the metadata stores.

### 1. Prepare data dictionary JSON files

One JSON file per table, following this schema:
...
Supported `LLMservice` values: `open_ai`, `anthropic`, `google`, `groq`.
...
| `column_score_threshold` | `0.0` | Columns with a BM25 score below this are dropped (except key columns). Raise to reduce prompt size. |

### Prompt types (`PromptBuilder`)

| Type | Parameters | Description |
|------|-----------|-------------|
| `generate sql` | `SCHEMA` | Main SQL generation prompt. Context formatted by `PromptBuilder.format_schema()`. |
...
```
tests/test_prompt_builder.py   # format_schema(), build() behaviour
tests/test_filters.py          # reranker filtering, BM25 column scoring
tests/test_base_utils.py       # SQLite CRUD, cachefunc, interface conformance
```
...
3. In `RAGPipeline.ManageInformation.initialize_client()`, instantiate your class instead of `ChromaVectorStore`
