...
|----|--------|------|------------|-------|
| CI1 | [ ] | **Re-enable PR check** — uncomment the `pull_request` trigger in `ci.yml` so tests must pass before any PR can be merged into `master`; this is the single highest-leverage CI change and unblocks all others | Low | `.github/workflows/ci.yml` |
| CI2 | [ ] | **Add ruff lint step** — `pyproject.toml` already has ruff config; add a `ruff check .` step to the CI job so style/import errors are caught on every PR before tests even run | Low | `.github/workflows/ci.yml` |
