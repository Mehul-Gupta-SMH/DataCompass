...
| P4 | [ ] | **Batch schema fetching** — preload all tool-requested table schemas in one DB round-trip per gather cycle rather than one query per `get_schema` call | Low |
...
| A2 | [ ] | **Replace NetworkX pickle with a graph DB** — move relation data to Neo4j or similar for concurrent-safe, scalable graph queries; pickle is not write-safe under concurrency | High |
...
### Requirements & Plan (Codex)
- Requirement (2026-03-11): Review Claude's workspace changes, focus on the latest DataLineage/Join Path tweaks, rerun the frontend suites, and log the results so the commit history reflects the actual validation status.
- Plan: inspect the current uildFlow tweaks, gain context on the join-type rendering, execute 
pm test -- App ChatInterface IngestTable, and capture both successes and the environment-level EPERM failures in this log.

### Actions Completed — 2026-03-11
- Confirmed uildFlow now accepts the optional joinTypeFilter, pulls the shared joinTypeColors map out of the body (still color-coding 1:1/1:n/n:1/n:m edges), and leaves the join-key labels intact.
- Tried 
pm test -- App ChatInterface IngestTable and the individual ChatInterface/IngestTable suites, but each run fails at esbuild while bundling ite.config.js because the child esbuild service cannot spawn (EPERM). Overriding ESBUILD_BINARY_PATH and redirecting the temp directory to ./tmp did not clear the restriction, so the Vitest logs stay red until the environment allows spawning the binary again.
